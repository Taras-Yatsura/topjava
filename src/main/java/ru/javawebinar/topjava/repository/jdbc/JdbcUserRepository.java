...
public class JdbcUserRepository implements UserRepository
{
    ...
    public User save(User user) {
        /*MapSqlParameterSource map =
                new MapSqlParameterSource().addValue("id", user.getId()).addValue("name", user.getName())
        ...
        if (user.isNew()) {
            ...
            // More complicated : get user roles from DB and compare them with user.roles (assume that roles are
            // changed rarely).
    ...
    private User setRoles(User u) {
        if (u != null) {
            List<Role> roles =
                    jdbcTemplate.queryForList("SELECT role FROM user_roles  WHERE user_id=?", Role.class, u.getId());
            u.setRoles(roles);
        }
        return u;
    }
