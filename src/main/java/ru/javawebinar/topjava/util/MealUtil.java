...
import ru.javawebinar.topjava.model.MealTrObj;
...
public class MealUtil
{
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException
    {
        ...
        List<MealTrObj> mealsTo = filteredByStreams(meals, startTime, endTime, 2000);
    ...
    public static List<MealTrObj> filteredByCycles(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                   int caloriesPerDay)
    {
        final Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        meals.forEach(meal -> caloriesSumByDate.merge(meal.getDate(), meal.getCalories(), Integer::sum));

        final List<MealTrObj> mealsTo = new ArrayList<>();
        meals.forEach(meal -> {
            if (isBetweenHalfOpen(meal.getTime(), startTime, endTime))
            {
                mealsTo.add(createTrObj(meal, caloriesSumByDate.get(meal.getDate()) > caloriesPerDay));
            }
        });
        return mealsTo;
    }

    public static List<MealTrObj> filteredByStreams(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                    int caloriesPerDay)
    {
    ...
    private static List<MealTrObj> filteredByRecursion(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                       int caloriesPerDay)
    {
        ArrayList<MealTrObj> result = new ArrayList<>();
        filterWithRecursion(new LinkedList<>(meals), startTime, endTime, caloriesPerDay, new HashMap<>(), result);
        return result;
    }

    private static void filterWithRecursion(LinkedList<Meal> meals, LocalTime startTime, LocalTime endTime,
                                            int caloriesPerDay, Map<LocalDate, Integer> dailyCaloriesMap,
                                            List<MealTrObj> result)
    {
        if (meals.isEmpty())
        { return; }
        ...
        if (isBetweenHalfOpen(meal.getTime(), startTime, endTime))
        {
            result.add(createTrObj(meal, dailyCaloriesMap.get(meal.getDate()) > caloriesPerDay));
        }
    }

    private static List<MealTrObj> filteredBySetterRecursion(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                             int caloriesPerDay)
    {
        class MealNode
        {
            private final MealNode prev;
            private final MealTrObj mealTo;

            public MealNode(MealTrObj mealTo, MealNode prev)
            {
                this.mealTo = mealTo;
                this.prev = prev;
            }
        ...
        List<MealTrObj> mealsTo = new ArrayList<>();
        meals.forEach(meal -> {
            ...
            {
                MealTrObj mealTo = createTrObj(meal, excess);
                mealsTo.add(mealTo);
                if (!excess)
                {
    ...
    private static List<MealTrObj> filteredByAtomic(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                    int caloriesPerDay)
    {
        Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        ...
        meals.forEach(meal -> {
            ...
            if (isBetweenHalfOpen(meal.getTime(), startTime, endTime))
            {
                mealsTo.add(createTrObj(meal, wrapBoolean.get()));  // also change createTo and MealTo.excess
            }
        });
        return mealsTo;
    }

    private static MealTrObj createTrObj(Meal meal, boolean isExcess)
    {
        return new MealTrObj(meal.getDateTime(), meal.getDescription(), meal.getCalories(), isExcess);
    }

    private static List<MealTrObj> filteredByReflection(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                        int caloriesPerDay)
            throws NoSuchFieldException, IllegalAccessException
    {
        ...
        for (Meal meal : meals)
        {
            ...
            if (isBetweenHalfOpen(meal.getTime(), startTime, endTime))
            {
                mealsTo.add(createTrObj(meal, mutableBoolean));  // also change createTo and MealTo.excess
            }
        }
        return mealsTo;
    }

    private static List<MealTrObj> filteredByClosure(List<Meal> mealList, LocalTime startTime, LocalTime endTime,
                                                     int caloriesPerDay)
    {
        final Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        List<MealTrObj> mealsTo = new ArrayList<>();
        mealList.forEach(meal -> {
            ...
            if (isBetweenHalfOpen(meal.getTime(), startTime, endTime))
            {
                mealsTo.add(createTrObj(meal, (caloriesSumByDate.get(meal.getDate()) >
                                               caloriesPerDay))); // also change createTo and MealTo.excess
            }
        });
        return mealsTo;
    }

    private static List<MealTrObj> filteredByExecutor(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                      int caloriesPerDay)
            throws InterruptedException, ExecutionException
    {
        Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        List<Callable<Void>> tasks = new ArrayList<>();
        final List<MealTrObj> mealsTo = Collections.synchronizedList(new ArrayList<>());
    ...
    public static List<MealTrObj> filteredByLock(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                 int caloriesPerDay) throws InterruptedException
    {
        Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        List<MealTrObj> mealsTo = new ArrayList<>();
    ...
    private static List<MealTrObj> filteredByCountDownLatch(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                            int caloriesPerDay)
            throws InterruptedException, ExecutionException
    {
        Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        List<MealTrObj> mealsTo = Collections.synchronizedList(new ArrayList<>());
    ...
    public static List<MealTrObj> filteredByPredicate(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                      int caloriesPerDay)
    {
        Map<LocalDate, Integer> caloriesSumByDate = new HashMap<>();
        List<MealTrObj> mealsTo = new ArrayList<>();
        ...
        for (Meal meal : meals)
        {
            ...
            {
                predicate = predicate.and(b -> mealsTo.add(createTrObj(meal, caloriesSumByDate.get(meal.getDateTime()
                                                                                                       .toLocalDate()) >
                                                                             caloriesPerDay)));
            }
        }
    ...
    public static List<MealTrObj> filteredByConsumerChain(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                          int caloriesPerDay)
    {
        Map<LocalDate, Integer> caloriesPerDays = new HashMap<>();
        List<MealTrObj> result = new ArrayList<>();
        ...
        for (Meal meal : meals)
        {
            ...
            if (TimeUtil.isBetweenHalfOpen(meal.getTime(), startTime, endTime))
            {
                consumer = consumer.andThen(dummy -> result.add(createTrObj(meal, caloriesPerDays.get(meal.getDateTime()
                                                                                                          .toLocalDate()) >
                                                                                  caloriesPerDay)));
            }
        }
    ...
    private static List<MealTrObj> filteredByFlatMap(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                     int caloriesPerDay)
    {
        Collection<List<Meal>> list = meals.stream().collect(Collectors.groupingBy(Meal::getDate)).values();

        return list.stream().flatMap(dayMeals -> {
            boolean excess = dayMeals.stream().mapToInt(Meal::getCalories).sum() > caloriesPerDay;
            return dayMeals.stream().filter(meal -> isBetweenHalfOpen(meal.getTime(), startTime, endTime))
                           .map(meal -> createTrObj(meal, excess));
        }).collect(toList());
    }

    private static List<MealTrObj> filteredByCollector(List<Meal> meals, LocalTime startTime, LocalTime endTime,
                                                       int caloriesPerDay)
    {
        final class Aggregate
        {
            ...
            private Stream<MealTrObj> finisher()
            {
                final boolean excess = dailySumOfCalories > caloriesPerDay;
                return dailyMeals.stream().map(meal -> createTrObj(meal, excess));
            }
        }

        Collection<Stream<MealTrObj>> values = meals.stream().collect(Collectors.groupingBy(Meal::getDate, Collector
                .of(Aggregate::new, Aggregate::accumulate, Aggregate::combine, Aggregate::finisher))).values();
